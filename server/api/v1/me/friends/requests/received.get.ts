...
const RECEIVED_FRIEND_REQUESTS_CACHE_KEY = 'profile-friends-requests-received'

export default defineEventHandler(async (event) => {
  const suffix = buildQuerySuffix(event)
  const cacheKey = buildProfileResourceCacheKey(RECEIVED_FRIEND_REQUESTS_CACHE_KEY, suffix)
  ...
  const requests = await proxyAuthApiGet(event, `/api/v1/me/friends/requests/received${suffix}`)
